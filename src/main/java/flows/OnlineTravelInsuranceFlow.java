...
public class OnlineTravelInsuranceFlow {
    ...
    private final TravelInsuranceStartPage startPage;
    private final TravelInsuranceIntroPage introPage;
    public final TravelInsurancePreviousQuestionPage previousQuestionPage;
    public final TravelInsurancePassengersQuestionPage passengersQuestionPage;
    public final TravelInsuranceDestinationSelectionPage destinationSelectionPage;
    public final TravelInsuranceDateSelectionPage dateSelectionPage;
    private final LoggerUtils loggerUtils;

    public OnlineTravelInsuranceFlow(BasePage basePage, LoggerUtils loggerUtils) {
       this.basePage = basePage;
        this.startPage = new TravelInsuranceStartPage(basePage);
        this.introPage = new TravelInsuranceIntroPage(basePage);
        this.previousQuestionPage = new TravelInsurancePreviousQuestionPage(basePage);
        this.passengersQuestionPage = new TravelInsurancePassengersQuestionPage(basePage);
        this.destinationSelectionPage = new TravelInsuranceDestinationSelectionPage(basePage);
        this.dateSelectionPage = new TravelInsuranceDateSelectionPage(basePage);
        this.loggerUtils = loggerUtils;
    }

    public void initFlowToPreviousInsuranceStep() throws Exception {
        startPage.scrollToTravelInsuranceButton();
        startPage.clickTravelInsuranceButton();
        basePage.switchToNewTab(); // ⬅️ הוסף את זה!
        introPage.scrollToContinue();
        introPage.clickContinue();
        introPage.scrollToContinue();
        introPage.clickContinue();
    }

    public void goToPassengersQuestionStep() {
        previousQuestionPage.clickFirstTimeOption();
    }

    public void goToDestinationSelectionStep() {
        passengersQuestionPage.clickYesOption();
    }

    public void completeDestinationSelection() {
        destinationSelectionPage.clickContinue();
    }

    public void selectEuropeAndContinue() {
        dateSelectionPage.selectEuropeDestination();
        dateSelectionPage.clickContinue();
    }

    // אפשרויות לבדיקה בשלבים השונים
    public boolean isErrorDisplayedAfterNoPassengers() {
        return passengersQuestionPage.isErrorMessageVisible();
    }

    public boolean isBackButtonDisplayedOnError() {
        return passengersQuestionPage.isBackToHomeButtonVisible();
    }

    public boolean areDatesFieldsVisible() {
        return dateSelectionPage.isStartDateFieldVisible() && dateSelectionPage.isEndDateFieldVisible();
    }

    public void clickAlreadyPurchasedOption() {
        previousQuestionPage.clickAlreadyPurchasedOption();
    }
}
