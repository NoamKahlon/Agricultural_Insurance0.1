...
//    @BeforeMethod
//    public void setup(){
//        flow = new OnlineTravelInsuranceFlow(basePage,loggerUtils);
//    }
//
...
public class OnlineTravelInsuranceTests extends Base {
    ...
    public void test_StartInsuranceFlow() throws Exception {
        try {
            flow.initFlowToPreviousInsuranceStep();
        } catch (Exception e) {
            loggerUtils.log("❌ Test failed: " + e.getMessage(), "StartFlowFail", false, true);
            throw e;
        }
    ...
    public void test_PreviousInsuranceQuestion() throws Exception {
        try {
            flow.initFlowToPreviousInsuranceStep();
            Assert.assertTrue(flow.previousQuestionPage.isFirstTimeOptionVisible());
            Assert.assertTrue(flow.previousQuestionPage.isAlreadyPurchasedOptionVisible());
        } catch (Exception e) {
            loggerUtils.log("❌ Test failed: " + e.getMessage(), "PreviousQuestionFail", false, true);
            throw e;
        }
    ...
    public void test_PassengersFromIsraelQuestion() throws Exception {
        try {
            flow.initFlowToPreviousInsuranceStep();
            flow.goToPassengersQuestionStep();
            Assert.assertEquals(flow.passengersQuestionPage.getQuestionText().trim(), "האם כל הנוסעים יוצאים מישראל");
        } catch (Exception e) {
            loggerUtils.log("❌ Test failed: " + e.getMessage(), "PassengersQuestionFail", false, true);
            throw e;
        }
    ...
    public void test_QuoteWithPreviousInsurance() throws Exception {
        try {
            flow.initFlowToPreviousInsuranceStep();
            flow.clickAlreadyPurchasedOption();

            boolean phoneFieldDisplayed = flow.dateSelectionPage.isStartDateFieldVisible();
            boolean idFieldDisplayed = flow.dateSelectionPage.isEndDateFieldVisible();

            loggerUtils.log(phoneFieldDisplayed ? "✅ Start date field is visible" : "❌ Start date field is NOT visible",
                    "", phoneFieldDisplayed, false);

            loggerUtils.log(idFieldDisplayed ? "✅ End date field is visible" : "❌ End date field is NOT visible",
                    "", idFieldDisplayed, false);
    ...
    public void test_BlockFlowWhenPassengersNotFromIsrael() throws Exception {
        try {
            flow.initFlowToPreviousInsuranceStep();
            flow.goToPassengersQuestionStep();
            flow.passengersQuestionPage.clickNoOption();

            boolean errorDisplayed = flow.isErrorDisplayedAfterNoPassengers();
            loggerUtils.log(errorDisplayed ? "✅ Error message visible" : "❌ Error message missing", "", errorDisplayed, false);

            boolean backButtonDisplayed = flow.isBackButtonDisplayedOnError();
            loggerUtils.log(backButtonDisplayed ? "✅ 'Back to homepage' button visible" : "❌ 'Back to homepage' button missing", "", backButtonDisplayed, false);
    ...
    @Test(description = "TC-043 - Verify online travel insurance quote flow is accessible and displayed correctly")
    @Description("Navigates to the online travel insurance quote page on Bituach Haklai site, verifies correct UI flow and elements for stage 1: destination selection")
    public void test_VerifyDestinationSelectionStep() {
        try {
            flow.initFlowToPreviousInsuranceStep();
            flow.goToPassengersQuestionStep();
            flow.goToDestinationSelectionStep();
            flow.completeDestinationSelection();
            Assert.assertTrue(flow.destinationSelectionPage.isContinueButtonVisible());
        } catch (Exception e) {
            loggerUtils.log("❌ Test failed: " + e.getMessage(), "TC043_Failure", false, true);
        }
    }

    @Test(description = "TC-044 - Verify date selection step in travel insurance quote flow")
    @Description("Navigates to the online travel insurance quote on Bituach Haklai site, completes destination step, and verifies correct UI and actions for stage 2: date selection (departure and return)")
    public void test_VerifyDateSelectionStep() {
        try {
            flow.initFlowToPreviousInsuranceStep();
            flow.goToPassengersQuestionStep();
            flow.goToDestinationSelectionStep();
            flow.completeDestinationSelection();
            flow.selectEuropeAndContinue();

            boolean isStartVisible = flow.dateSelectionPage.isStartDateFieldVisible();
            boolean isEndVisible = flow.dateSelectionPage.isEndDateFieldVisible();
            ...
            loggerUtils.log("❌ Test failed: " + e.getMessage(), "TC044_Failure", false, true);
        }
    }
}